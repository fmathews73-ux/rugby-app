...
| 28 | Image rights — union crests (RFU, FFR, NZR, IRFU, WRU, SRU, SARU, UAR, JRFU, WR itself, etc.), club logos (Premiership / Top 14 / URC / Super Rugby clubs), national team crests (springbok, silver fern, shamrock, thistle, dragon, three feathers, etc.), coaching-staff photos, player photos. Bundled with the chosen provider's licence tier at real-data cutover WHERE OFFERED — Sportradar sells an images product via agency partnerships, but Stats Perform's public materials advertise NO rugby image package (research 2026-07-06: their Opta Graphics product expects CUSTOMERS to upload their own licensed headshots/crests). If Opta is the data pick, images become a separate line item: photo-agency licence for headshots (Getty / PA / Inpho are the rugby specialists) + direct union/tournament licences for crests (trademarks — never resold by data providers). Determines whether the app can show any crest / logo beyond neutral placeholders + national flags, or stays on the current placeholder-and-flag-only default (root `CLAUDE.md` §9). Independent of register #5 (data redistribution licence) — image rights are a separate licence tier and can fail even when the data licence clears. | RESEARCH | 6 (pre-launch cutover) | Team detail visuals, fixture-header visuals, in-app branding beyond flags. Tied to register #6 (provider selection) and register #7 (ancillary data availability). |
| 29 | Analysis narrative composition standard — `docs/analysis-composition-guide.md` (data → analysis → insight; no situation-only sentences, no mail-merge parallels, no space-filling symmetry). Initial composition pass over ALL four client-side templates (pre-match, match, team, player) COMPLETED 2026-07-05. Standing obligations: new narrative copy meets the guide on first writing; full corpus RE-VERIFIED against the guide as a hard precondition of the Phase 6 LLM cutover (spec §7 step 7 — guide + spec together form the system prompt, template outputs double as few-shot reference material). | BUILD (initial pass done; re-verify at cutover) | 6 (pre-cutover gate) | Analysis tabs on fixture / team / player drills, Home Team Analysis card, pre-match card. |
| 30 | Analysis gap-threshold re-tune at real-data cutover — the pre-match/match analysis engines' axis-gap thresholds (`GAP_POINTS=6`, `GAP_SET_PIECE=4`, `GAP_DISCIPLINE=2.5`, `GAP_SHARE=6`, `GAP_TURNOVERS=2`, kicking 15% relative, `TIMING_SKEW=35`; spec §11 threshold table) were sanity-checked against SYNTHETIC per-game variance, which has no team-strength model. Real feed data will shift the distributions (stabler profiles, more genuine dead-heats between matched sides, flat-track-bully inflation from unadjusted strength of schedule). Before Phase 6 cutover: re-derive thresholds from real historical distributions, and evaluate opponent-adjusted per-game rates as the longer-term fix. Until then the engine may systematically over- or under-call edges. | BUILD | 6 (pre-cutover gate) | Pre-Match analysis card + Gap Ladder, match-analysis Variance/axis sections, Keys derivation. |
| 31 | Freemium/subscription split — DIRECTION SET (owner, 2026-07-06): free tier gets the Pre-Match surface; everything else (Match Analysis, Stats, team/player analytical surfaces) gates on subscription. The exact chart-level and dataset-level split (which evidence pages, which stat rows, which narrative sections are free vs premium) is deliberately UNDECIDED — revisit once the app is fully developed, before beta pricing. Supersedes the older "Stats screen is premium-gated" framing (§8) as the primary gate model; the existing per-row `locked` mechanism on Stats becomes one instrument of the wider gate. | INPUT NEEDED (split details) | 5-6 (pre-beta) | Every analytical surface; paywall UX; server entitlement checks (client always trusts the server's entitlement decision). |
| 32 | Licensed design asset — flag-shield silhouette. Nation flags render app-wide inside a shield sticker whose vector geometry comes from owner-licensed Shutterstock asset #1166737669 (standard licence, acquired 2026-07-07; exact outer/inner paths extracted from the EPS into `mobile/src/components/team-flag-shield.tsx`). Flags themselves remain flagcdn sources. ACTION at Phase 6 store-submission diligence: confirm the Shutterstock standard licence covers in-app distribution at expected install volumes (enhanced licence if >500k reproductions per their terms). | BUILD (licence check at launch) | 6 (pre-launch) | Every flag rendering surface. |
| 33 | Aerial-contest data availability — the Aerial Contest surfaces (team-carousel card, pre-match pair card, gap-ladder rungs, engine narratives; built 2026-07-07) consume per-match contestable-kick outcomes: contestables PUT UP per side + regathers won (receptions derive from the opponent's delivered numbers; `Result.home/away_contestable_kicks[_won]`). These are currently SYNTHETIC (generator band: 4-9 contestables/game, 25-60% regather). RESEARCH REQUIRED at Phase 6 provider selection (with #6/#7): confirm the chosen feed supplies aerial-contest/kick-regather event outcomes at team-match granularity — Stats Perform's advanced rugby feed is believed to carry them, but this is exactly the metric class that sits in a premium feed tier. If unavailable at the licensed tier, the aerial surfaces must degrade gracefully (hide, never fabricate — §5.5). Tier-1 baselines hard-coded in the client (43%/57% + volumes) must be re-derived from real pool averages at cutover, same as #30's thresholds. | RESEARCH REQUIRED | 6 (provider selection) | Aerial Contest cards (team + pre-match), gap ladder, pre-match keys/shape phrases, `use-team-analysis.aerial`. |
| 34 | Pitch Heatmap — REMOVED from Match Analysis (2026-07-08). The synthetic heat gradient (derived from coarse event x positions) restated the Kicking & Territory numbers with less precision, failing the "does it earn its card" test; its territory narrative paragraph now rides Kicking & Territory's flip back. REVISIT at Phase 6 real-data cutover: with genuine per-event pitch coordinates (`MatchEvent.x` is already in the schema; verify the feed supplies real x/y at licensed tier alongside #33) a pitch heatmap becomes a differentiated visual worth restoring — real pressure zones, 22-entry maps, kick landing zones. Component deleted (git history is the archive: `mobile/src/components/insights/pitch-heatmap.tsx`, last at commit 0681ab4-era). | DEFERRED | 6 (real-data cutover) | Match Analysis carousel; `use-match-analysis.heatmap` narrative (currently folded into Kicking & Territory). |
| 35 | Licensed design asset — brand logo mark. The header logo (molecule/network mark left of the RUGBYMETRICS wordmark) uses vector geometry from owner-licensed Shutterstock asset #1758547613 (standard licence, acquired 2026-07-08; exact united-silhouette path extracted from the EPS into `mobile/src/components/app-logo.tsx`). Same Phase 6 diligence as #32: confirm the standard licence covers in-app distribution at expected install volumes, and note logo marks inside an app icon / store listing may need the enhanced tier. Colour is the asset red as a brand placeholder — final palette rides register #23. | BUILD (licence check at launch) | 6 (pre-launch) | App header; future app icon / splash / store assets. |
